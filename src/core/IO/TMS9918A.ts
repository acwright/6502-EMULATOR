...
/**
 * TMS9918 Video Display Processor Emulation — the TMS9918A of emulator 2.7.0,
 * restored.
 *
 * This is 2.7.0's `Video.ts` with the class renamed, `model` and
 * `registerCount` added, `frameIndices()` added (from `v3-vdp` 5a8a614), and the
 * display size taken from `VideoCard.ts`. Nothing else: it is the 2.x card byte
 * for byte, held to the TMS9918A goldens, and it is fixed, never extended. The
 * 6502-PICOVDP is `Video.ts`.
 *
...
export class TMS9918A implements IO {

  /**
   * The same kind as the PICOVDP's, so that a version 1 snapshot (from 2.7.0)
   * still deserializes into this card. Snapshots tell the two apart by their
   * top-level `vdp`, not by the slot's kind.
   */
  readonly kind = 'video'

  /** Which card this is (`--vdp tms9918a`). */
  readonly model = 'tms9918a' as const

  /** Eight registers, `getRegister(0)` to `getRegister(7)`. */
  readonly registerCount = TMS_NUM_REGISTERS
