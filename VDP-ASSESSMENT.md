...
- **ROM follows the card.** Bundle BIOS 1.6 and, once released, BIOS 2.x.
  ...
  - 2.x on a TMS9918A card must behave as 6502-BIOS decides (its assessment, B item 4).
    Test that combination too.
...
|---|---|---|
| 6502-PICOVDP (`~/Developer/C/6502-PICOVDP`) | `tools/sync-oracle.mjs` requires this repo on branch `v3-vdp`. `host/node/Video.cjs` stands in for `src/core/IO/Video` via `jest.picovdp.cjs` | Any `bios/` golden re-capture (1.6, 2.0); merging the branch; renaming or moving `Video.ts`. Coordinate and re-sync the oracle |
